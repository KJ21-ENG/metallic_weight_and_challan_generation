...
import { generateStickerHTML, DEFAULT_STICKER_HEADER } from './stickerLayout';
/**
 ...
 */
export async function printLabel(labelData) {
    try {
 ...
 */
async function silentPrintBoxSticker(boxData, rollId, customerName, selectedLotDetails) {
    ...
    `;
        ...
        const stickerData = {
            ...
            firmName: boxData.firmName || DEFAULT_STICKER_HEADER // Use firm name or fallback to default
        };
        ...
        if (window.electron) {
            try {
                ...
                const printResult = await window.electron.ipcRenderer.invoke('print-silently', {
                    html: html,
                    silent: true,
        ...
        if (loadingOverlay) {
            const errorMessage = loadingOverlay.querySelector('.loading-message');
            if (errorMessage) {
                errorMessage.textContent = 'Error: ' + (error.message || 'Print Failed');
                errorMessage.classList.remove('text-gray-800', 'text-green-600');
                errorMessage.classList.add('text-red-600');
            }
            const progressBarContainer = loadingOverlay.querySelector('.progress-bar-container');
